...
from Options import Choice, Range, Toggle, OptionGroup, PerGameCommonOptions, StartInventoryPool, DeathLink
...
class BossCard5(Choice):
    """Determines what item should be placed on the fifth gold card for bosses
    Anything - this location will be treated like any other location
    Kill_count_reduction - this location will always contain a kill count reduction item, 
    when this option is not selected they will be moved into the item pool instead
    Filler - will place an unimportant filler item on these locations effectively excluding them"""
    option_anything = 0
    option_kill_count_reduction = 1
    option_filler = 2
    default = 1
...
bomberman_64_option_groups = [
    ...
    OptionGroup("Bomberman Model Options", [
        BombermanColor,
        BomberHead,
        BomberBody,
        BomberArmLeft,
    ...
    OptionGroup("Lua Script Only Options", [
        RandomEnemyAI,
        RandomEnemyModel,
    ])
]
