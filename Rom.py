...
def write_tokens(world:World, patch:Bomb64ProcedurePatch):
    ...
    arm_left = world.options.arm_left_part.value
    leg_left = world.options.leg_left_part.value
    if world.options.arm_right_part.value == 0xFE:
        arm_right = arm_match[arm_left]
    else:
        arm_right = world.options.arm_right_part.value

    if world.options.leg_right_part.value == 0xFE:
        leg_right = leg_match[leg_left]
    else:
        leg_right = world.options.leg_right_part.value

    patch.write_token(APTokenTypes.WRITE, 0xDFFB9, bytes([world.options.head_part.value]))
    patch.write_token(APTokenTypes.WRITE, 0xDFFBA, bytes([world.options.body_part.value]))
    patch.write_token(APTokenTypes.WRITE, 0xDFFBB, bytes([arm_left]))
    patch.write_token(APTokenTypes.WRITE, 0xDFFBC, bytes([arm_right]))
    patch.write_token(APTokenTypes.WRITE, 0xDFFBD, bytes([leg_left]))
    patch.write_token(APTokenTypes.WRITE, 0xDFFBE, bytes([leg_right]))
    #patch.write_token(APTokenTypes.WRITE, 0xDFFB8, bytes([world.team]))
    ...
    # Do Not Gain Bombs
    #patch.write_token(APTokenTypes.WRITE, 0x9232C, bytes([0x01, 0x40, 0x58, 0x21]))
    patch.write_token(APTokenTypes.WRITE, 0x94108, NOP)
    # Do Not Gain Fireups
    #patch.write_token(APTokenTypes.WRITE, 0x936BC, bytes([0x01, 0x40, 0x58, 0x21]))
    # This frees up some space
    patch.write_token(APTokenTypes.WRITE, 0x940C8, NOP)

    # Do Not Gain Remote Bomb
    # patch.write_token(APTokenTypes.WRITE, 0x941CF, bytes([0x00]))
    ...
    #patch.write_token(APTokenTypes.WRITE, 0x922A0, bytes([FREESPACE]))
    ...
    #patch.write_token(APTokenTypes.WRITE, 0x166A5F, bytes([0x01]))

    # Open Black Fortress
    #patch.write_token(APTokenTypes.WRITE, 0x166A5F, bytes([0x8C, 0x43, 0x01,0x64])),
    ...
    patch.write_token(APTokenTypes.WRITE, 0x2376, bytes([0x0D, 0x54]))
    patch.write_token(APTokenTypes.WRITE, 0x593A0, bytes([0x0C, 0x0B, 0x40, 0x24])) # JAL 0x802D0090
    patch.write_token(APTokenTypes.WRITE, 0x60554, bytes([0x0C, 0x0B, 0x40, 0x24])) # JAL 0x802D0090
    patch.write_token(APTokenTypes.WRITE, 0x78500, bytes([0x0C, 0x0B, 0x40, 0x24])) # JAL 0x802D0090
    patch.write_token(APTokenTypes.WRITE, 0x4150, bytes(CUSTOM_PART_01))
    patch.write_token(APTokenTypes.WRITE, 0x7B75A4, bytes(CUSTOM_PART_02))

    #if world.options.head_part.value != 0xFF:
    #    patch.write_token(APTokenTypes.WRITE, 0xBB3A8, bytes([0x00, 0x00, 0x00, world.options.head_part.value]))
    #if world.options.body_part.value != 0xFF:
    #    patch.write_token(APTokenTypes.WRITE, 0xBB3D4, bytes([0x00, 0x00, 0x00, world.options.body_part.value]))
    #if world.options.arm_left_part.value != 0xFF:
    #    patch.write_token(APTokenTypes.WRITE, 0xBB400, bytes([0x00, 0x00, 0x00, world.options.arm_left_part.value]))
    #if world.options.arm_right_part.value != 0xFF:
    #    patch.write_token(APTokenTypes.WRITE, 0xBB42C, bytes([0x00, 0x00, 0x00, world.options.arm_right_part.value]))
    #if world.options.leg_left_part.value != 0xFF:
    #    patch.write_token(APTokenTypes.WRITE, 0xBB458, bytes([0x00, 0x00, 0x00, world.options.leg_left_part.value]))
    #if world.options.leg_right_part.value != 0xFF:
    #    patch.write_token(APTokenTypes.WRITE, 0xBB484, bytes([0x00, 0x00, 0x00, world.options.leg_right_part.value]))
