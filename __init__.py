...
from .Locations import Bomb64Location, location_data_table, location_table, locked_locations, normal_only_location_data, normal_location_table, boss_fifth_list#, hard_only_location_data, hard_location_table
from .Options import Bomb64Options, bomberman_64_option_groups
...
class Bomb64WebWorld(WebWorld):
    ...
    tutorials = [setup_en]
    option_groups = bomberman_64_option_groups

class Bomb64World(World):
    """Bomberman 64 is a puzzle game with a few 3D-action coats of paint on top. 
    Red Bombs (Called Power Bombs in the apworld) and remote bombs are now permanent items along with the 'Power Glove' and 'Bomb Kick' abilities.
    Stage Progression is also now gated by finding 'Key' items to unlock new stages."""
    ...
    def create_items(self) -> None:
        ...
        if self.options.boss_card_five.value != 1:
            for x in range(9):
                item_pool.append(self.create_item("Kill Count Reduction"))
    ...
    def create_regions(self) -> None:
        ...
        bosscard5 = self.options.boss_card_five.value
        ...
        if bosscard5 == 1: # kill_count_reduct
            for loc_name in boss_fifth_list:
                self.get_location(loc_name).place_locked_item(self.create_item("Kill Count Reduction"))
        elif bosscard5 == 2: # Filler
            for loc_name in boss_fifth_list:
